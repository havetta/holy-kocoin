...
export function runWebSocket() {
  ...
  if (exchangeName === "bybit") url = `wss://stream.bybit.com/v5/public/spot`;
  ...
  ws.on("message", function incoming(data) {
    try {
      ...
      if (state.recentPrices.length > 30)
        state.recentPrices.shift();

      const tick = rightPad(Math.random(), 5);
      oneLine(tick, twoDecimals(state.avgPrice), twoDecimals(state.curPrice), `Recent0-5: ${twoDecimals(recentPriceAvg(0, 15))}   recent5: ${twoDecimals(recentPriceAvg(-15, 15))}   Buy: ${twoDecimals(state.buyPrice)}      ${state.balanceStr}`);
    } catch (e) {
      err(e?.message);
      warn(e?.stack);
    }
